...
function DocumentConversion(options, comments) {
    ...
    function convertRun(run, messages, options) {
        ...
        var paths = [];
        if (run.color && colorClassMap[run.color]){
            paths.push(htmlPaths.element("span", { class : colorClassMap[run.color]}, {fresh: false}));
        }
    ...
    function convertTableCell(element, messages, options) {
        ...
        //Remove paragraph element for table cells with only one child element
        if(children.length === 1 && children[0].tag.tagName === "p"){
