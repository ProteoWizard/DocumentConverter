...
//main(options);
//main(inPlaceOptions);
//main(tutorialSandbox);
//main(invariantFolderOptions);
//main(postprocessOnly);
    ...
    files.filter(
        function(file) {
            ...
            fs.readFile(filePath, "utf8", (err, data) => {
                ...
                while(true){
                    processCounter++;
                    var newHtml = postProcessHtml(html)
                    if(newHtml === html){
                        break;
                    }
                    if(processCounter >= 30){
                        console.log(`Html processing reached limit of ${processCounter} iterations`)
                        throw `Html processing reached limit of ${processCounter} iterations`;
                    }
                    html = newHtml;
                }
...
function postProcessHtml(html){
    ...
    //remove empty line in p tag endings
    //html = html.replace(/(<p>[\s\S]*?)\s*\n\s*(<\/p>)/gs, '$1$2');
    return html;
}
...
function setDefaultOptions(options){
    ...
    if(!options.invariantFileNames) options.invariantFileNames = []
    if(options.inPlace === undefined) options.inPlace = false;
...
function convertDirectory(options, currentSourcePath, currentOutputPath, convertFunction){
    ...
    var files = fs.readdirSync(currentSourcePath, { recursive: options.inPlace });
    files.filter(
        function(file) {
...
function convertDocument(options, currentSourcePath, currentOutputPath, sourceDocument) {
    ...
    var outputFolder = options.inPlace ? path.parse(`${currentOutputPath}/${sourceDocument}`).dir: currentOutputPath + "/" + documentName;
